...
CALL :LOGHE-INFO Remove %CONFIG_CONFIG_FILEPATH%: %CONFIG_CONFIG_PATH%
REM echo.%CONFIG_CONFIG_BLK%

SET CONFIG_CENTER_FILE=%CONFIG_CONFIG_FILEPATH%

IF "%CONFIG_CONFIG_KEY%"=="#NONE" (
	CALL ccs.bat /send /blk-rm %CONFIG_CONFIG_BLK%
	GOTO :CONFIG-END
)

IF /i NOT "%CONFIG_CONFIG_BLK%"=="#NONE" (
	CALL ccs.bat /send /key-nblk-rm %CONFIG_CONFIG_KEY%
	GOTO :CONFIG-END
)

CALL ccs.bat /send /key-rm %CONFIG_CONFIG_BLK% %CONFIG_CONFIG_KEY%
...
:HELP
	ECHO.CALL CONFIG-rm.cmd /HELP
	ECHO.CALL CONFIG-rm.cmd ^<PATH^> {^<BLOCK^>^|#NONE}/{[KEY]^|#NONE}
	ECHO.CALL CONFIG-rm.cmd #PIDMD ^<BLOCK^>/{[KEY]^|#NONE}
	ECHO.CALL CONFIG-rm.cmd #SRV ^<SRV:name^> ^<KEY^>
	ECHO.
