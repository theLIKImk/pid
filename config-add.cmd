...
CALL :LOGHE-INFO Add %CONFIG_CONFIG_FILEPATH%: %CONFIG_CONFIG_PATH% SET %CONFIG_CONFIG_KEY_VAL%
...
:HELP
	ECHO.CALL CONFIG-ADD.cmd /HELP
	ECHO.CALL CONFIG-ADD.cmd ^<PATH^> {^<BLOCK^>^|#NONE}/^<KEY^> ^<NEW_VAL^>
	ECHO.CALL CONFIG-ADD.cmd #PIDMD ^<BLOCK^>/^<KEY^> ^<NEW_VAL^>
	ECHO.CALL CONFIG-ADD.cmd #SRV ^<SRV:name^> ^<KEY^> ^<NEW_VAL^>
	ECHO.
